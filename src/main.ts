...
import { SnakeGame, SnakeGameConfig } from "./commands/snake";
import { TetrisGame } from "./commands/tetris";
...
const COMMANDS = ["help", "about", "projects", "whoami", "repo", "banner", "snake", "tetris", "gui", "clear"];
...
function commandHandler(input : string) {
  ...
  switch(input) {
    case 'gui':
      ...
      setTimeout(() => {
        window.open('https://cyouryuu.github.io/', '_blank');
      }, 500);
      break;
    case 'clear':
      setTimeout(() => {
    ...
    case 'sudo':
      ...
      if (isSudo) {
        writeLines(["Permission already granted.", "<br>"]);
      } else {
        if(!PASSWORD) return
    ...
    case 'tetris':
      if(bareMode) {
        writeLines(["Don't play Russian game.", "<br>"]);
        break;
      }
      startTetrisGame();
      break;
...
function startTetrisGame() {
  ...
  canvas.width = 320; // 设置 canvas 宽度
  canvas.height = 320; // 设置 canvas 高度
  canvas.style.border = "5px solid #A9A9A9"; // 可以添加边框以便在网页中显示游戏区域
...
function startSnakeGame() {
  ...
  canvas.style.border = "5px solid #A9A9A9"; // 可以添加边框以便在终端中显示游戏区域
  ...
  const snakeGameConfig: SnakeGameConfig = {
    gridSize: 15,
    speed: 200,
  };

  const snakeGame = new SnakeGame(canvas, snakeGameConfig);
  snakeGame.run();
}
